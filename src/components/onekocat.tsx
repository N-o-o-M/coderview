...
// Define types for the sprite positions
type SpritePosition = [number, number];
type SpriteSet = Record<string, SpritePosition[]>;

// Define types for position objects
interface Position {
  x: number;
  y: number;
}
...
export default function OnekoCat(): JSX.Element {
  ...
  const [idleAnimationFrame, setIdleAnimationFrame] = useState<number>(0);
